...
Watching starts once the plan is readable, because only that exact plan can define the graph — after the invocation returns for `build` and `test`, partway through it for `run`, which builds and runs in one step. swift-watch records when the invocation began, starts the watcher, then scans the new graph for files modified in the gap, reaching a second further back to cover filesystems that stamp whole seconds. Any such edit causes an immediate follow-up cycle rather than being lost. The root `Package.resolved` is reconciled against the plan's own timestamp instead, since SwiftPM may rewrite it while resolving that plan. `Package.swift` keeps the invocation boundary because it may be edited after SwiftPM read it but before the plan was written.
